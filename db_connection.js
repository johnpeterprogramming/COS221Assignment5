...
class Database {
    ...
    getMovies(filters, callback) {
        ...
        let sql = "SELECT DISTINCT c.CatalogID, c.Title, c.Director, c.ReleaseDate, c.PosterUrl, m.Duration, g.Description as Genre FROM movies AS m, catalog AS c, genre AS g WHERE m.CatalogID = c.CatalogID AND c.CatalogID = g.CatalogID";
    ...
    getShows(filters, callback) {
        let sql = "SELECT DISTINCT c.CatalogID, c.Title, c.Director, c.ReleaseDate, c.PosterUrl, s.Seasons, s.Episodes FROM shows as s, catalog as c WHERE s.CatalogID = c.CatalogID";
